...
def fetch_cart_data(request):
    ...
    for item in cart_data:
        ...
        cart_html += '<td><button type="button" class="btn" onclick="decreaseQuantity({})">-</button></td>'.format(item['id'])
        cart_html += '<td id="quantity_{}">{}</td>'.format(item['id'], item['quantity'])
        cart_html += '<td><button type="button" class="btn" onclick="increaseQuantity({})">+</button></td>'.format(item['id'])
        cart_html += '</tr>'
    cart_html += '</table>'
...
def update_cart(request):
    if request.method == 'POST':
        ...
        new_quantity = next((item['quantity'] for item in cart_data if item['id'] == item_id), None)
